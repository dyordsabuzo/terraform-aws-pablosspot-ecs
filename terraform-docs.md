...
| <a name="input_autoscaling_metric_alarms"></a> [autoscaling\_metric\_alarms](#input\_autoscaling\_metric\_alarms) | Cloudwatch metric alarms associated with autoscaling | <pre>list(object({<br/>    identifer           = optional(string)<br/>    threshold           = optional(number)<br/>    comparison_operator = optional(string)<br/>    evaluation_periods  = optional(number)<br/>    period              = optional(number)<br/>    metric_name         = optional(string)<br/>    statistic           = optional(string)<br/>    unit                = optional(string)<br/>    metric_is_high      = bool<br/>  }))</pre> | <pre>[<br/>  {<br/>    "identifer": "cpu-high",<br/>    "metric_is_high": true,<br/>    "metric_name": "CPUUtilization",<br/>    "statistic": "Average",<br/>    "threshold": 70<br/>  },<br/>  {<br/>    "identifer": "cpu-low",<br/>    "metric_is_high": false,<br/>    "metric_name": "CPUUtilization",<br/>    "statistic": "Average",<br/>    "threshold": 30<br/>  },<br/>  {<br/>    "identifer": "memory-high",<br/>    "metric_is_high": true,<br/>    "metric_name": "MemoryUtilization",<br/>    "statistic": "Average",<br/>    "threshold": 70<br/>  },<br/>  {<br/>    "identifer": "memory-low",<br/>    "metric_is_high": false,<br/>    "metric_name": "MemoryUtilization",<br/>    "statistic": "Average",<br/>    "threshold": 30<br/>  }<br/>]</pre> | no |
| <a name="input_cluster"></a> [cluster](#input\_cluster) | ECS cluster properties | <pre>object({<br/>    name                      = string<br/>    create                    = optional(bool)<br/>    enable_container_insights = optional(bool)<br/>  })</pre> | n/a | yes |
| <a name="input_container_capacity"></a> [container\_capacity](#input\_container\_capacity) | Auto scaling container capacity properties | <pre>object({<br/>    min = number<br/>    max = number<br/>  })</pre> | <pre>{<br/>  "max": 4,<br/>  "min": 1<br/>}</pre> | no |
...
| <a name="input_container_healthcheck"></a> [container\_healthcheck](#input\_container\_healthcheck) | Health check settings | <pre>object({<br/>    path                = string<br/>    protocol            = string<br/>    interval            = number<br/>    healthy_threshold   = number<br/>    unhealthy_threshold = number<br/>    matcher             = string<br/>  })</pre> | <pre>{<br/>  "healthy_threshold": 2,<br/>  "interval": 5,<br/>  "matcher": "200-299,301-399",<br/>  "path": "/",<br/>  "protocol": "HTTP",<br/>  "unhealthy_threshold": 5<br/>}</pre> | no |
...
| <a name="input_scaling_adjustment"></a> [scaling\_adjustment](#input\_scaling\_adjustment) | Auto scaling adjustment parameters | <pre>object({<br/>    scale_up = object({<br/>      adjustment_type         = string<br/>      scaling_adjustment      = number<br/>      metric_aggregation_type = string<br/>    })<br/>    scale_down = object({<br/>      adjustment_type         = string<br/>      scaling_adjustment      = number<br/>      metric_aggregation_type = string<br/>    })<br/>  })</pre> | <pre>{<br/>  "scale_down": {<br/>    "adjustment_type": "ChangeInCapacity",<br/>    "metric_aggregation_type": "Maximum",<br/>    "scaling_adjustment": -1<br/>  },<br/>  "scale_up": {<br/>    "adjustment_type": "ChangeInCapacity",<br/>    "metric_aggregation_type": "Maximum",<br/>    "scaling_adjustment": 1<br/>  }<br/>}</pre> | no |
